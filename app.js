...
async function callOracleAgent(userMessage, conversationId) {
  try {
    ...
    for (var i = 0; i < 20; i++) {
      ...
      if (status === 'COMPLETED') {
        var reply = '';
        if (statusRes.data.message && statusRes.data.message.content && statusRes.data.message.content[0]) {
          reply = statusRes.data.message.content[0].text;
        } else if (statusRes.data.output && statusRes.data.output.content && statusRes.data.output.content[0]) {
          reply = statusRes.data.output.content[0].text;
        } else {
