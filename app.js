...
async function callOracleAgent(userMessage, conversationId) {
  try {
    ...
    console.error('Agent error status:', err.response ? err.response.status : 'no status');
    console.error('Agent error data:', JSON.stringify(err.response ? err.response.data : err.message));
    console.error('Agent error headers:', JSON.stringify(err.response ? err.response.headers : {}));
...
app.post('/webhook', async function(req, res) {
  res.sendStatus(200);
  try {
    var entry = req.body && req.body.entry && req.body.entry[
            0
        ];
    var change = entry && entry.changes && entry.changes[
            0
        ];
    var value = change && change.value;
    var message = value && value.messages && value.messages[
            0
        ];
    if (!message || message.type !== 'text') return;
    var userPhone = message.from;
    var userText = message.text.body;
    var convId = sessions[userPhone
        ] || null;
